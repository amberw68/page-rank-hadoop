...
////	public static final String CACHE_DIR = "D:\\Project\\Java\\CS549\\pagerank\\pageranksrc"; // for local
////	public static final String CACHE_DIR = "/cache"; 
//	
//	public static final String CACHE_DIR = "/cache";  // for local
//	//public static final String CACHE_DIR = "s3://yifanwang"; // for EMR
//	
...
////	}
	...
	static void init(String input, String output, int reducers)
		...
		System.out.println("Init Job Started by Yifan Wang");
	...
	static void iter(String input, String output, int reducers)
			throws IOException, ClassNotFoundException, InterruptedException {
		System.out.println("Iter Job Started by Yifan Wang");
		Job job = Job.getInstance(); // Creates a new Job
		job.setJarByClass(PageRankDriver.class); // Sets the Driver class
		job.setNumReduceTasks(reducers); // Sets the number of reducers
	...
	static void diff(String input1, String input2, String output, int reducers)
			throws Exception {
		System.out.println("Diff Job Part 1 Started by Yifan Wang");
		Job job = Job.getInstance(); // Creates a new job
	...
	static void finish(String input, String output, int reducers)throws Exception {
		...
		System.out.println("Finish Job Started by Yifan Wang");
		Job job = Job.getInstance(); // Creates a new Job
		job.setJarByClass(PageRankDriver.class); // Sets the Driver class
		job.setNumReduceTasks(reducers); // Sets the number of reducers
...
//		job.setMapOutputValueClass(Text.class);
//		
//		System.out.println(job.waitForCompletion(true) ? "Finish Job Completed" : "Finish Job Erro");
//		deleteDirectory(CURRENT_PATH + "/tempJoin");
...
//	}
	...
	public static void composite(String input, String output, String interim1,
		 ...
		 * TODO 
		 */
		System.out.println("Yifan Wang (10431233)");
						...
						// is the input directory
		{
			...
			counter++;
			if (nameInput != null) {
				convert(nameInput, interim2, interim1, reducers);
...
//			summarizeResult(output);
		} else // for even i, interim1 is the input directory
		{
			...
			counter++;
			if (nameInput != null) {
				convert(nameInput, interim1, interim2, reducers);
				finish(interim2, output, reducers);
			} else {
				finish(interim1, output, reducers);
			}
...
//			summarizeResult(output);
	...
	static void convert(String names, String input, String output, int reducers) throws Exception {
	    ...
	    job.setReducerClass(ConvertReduce.class);
